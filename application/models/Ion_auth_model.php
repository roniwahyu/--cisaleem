...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_hybrid($password, $salt=false, $use_sha1_override=FALSE)
	{
		if (empty($password))
		{
			return FALSE;
	...
	public function batchregister($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		$ip_address = $this->_prepare_ip($this->input->ip_address());
		$salt       = $this->store_salt ? $this->salt() : FALSE;
		$passwordx   = $this->hash_password($password, $salt);
		$usernamex   = $this->hash_password($identity, $salt);

		// Users table.
		$data = array(
		    $this->identity_column   => $identity,
		    'username'   => $identity,
		    'username_hash'   => $usernamex,
		    'password'   => $password,
		    'password_hash'   => $passwordx,
		   	'email'      => $email,
		    'ip_address' => $ip_address,
		    'created_on' => time(),
		    'active'     => ($manual_activation === false ? 1 : 0)
		...
		// filter out any data passed that doesnt have a matching column in the users table
		// and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['users1'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['users1'], $user_data);

		$id = $this->db->insert_id($this->tables['users1'] . '_id_seq');
